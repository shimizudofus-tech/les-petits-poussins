...
export default function MoyenneCountExercise({ section = 'moyenne', onCorrect }) {
  ...
  const quiz = useMemo(() => {
    ...
    if (source.type === 'count') {
      const maxAnswer = source.difficulty === 1 ? 3 : 5
      const optionCount = source.difficulty === 1 ? 3 : 3
      return {
        type: 'count',
    ...
    return (
      <>
        ...
        <div className="chalkboard petite-chalkboard">
          <div className="count-scene petite-count-scene moyenne-count-scene">{quiz.scene}</div>
        </div>
        <AnswerButtons
  ...
  return (
    <>
      <PetiteExerciseHeader
        ...
        audioKey={quiz.audioKey}
      />
      <div className="moyenne-compare-groups flex justify-center gap-4">
        {[
          ...
          >
            <div className="moyenne-compare-scene">{group.emoji.repeat(group.count)}</div>
